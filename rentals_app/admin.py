...
def create():
    ...
    if request.method == 'POST':
        ...
        rental.is_available = 1 if request.form.get(
            'is_available') == 'Yes' else 0
        rental.available_on = datetime.datetime.strptime(
            request.form.get('available_on'), '%Y-%m-%d').date()
...
def details(id):
    ...
    rental.implements = list(rental.implements.replace(
        ...
        if files[0].filename is None or files[0].filename == '':
            ...
            for file in files:
                if file:
                    ...
                    filename = secure_filename(file.filename)
                    file.save(helpers.ABS_UPLOAD_PATH +
                              folder_format+'/'+filename)
                    paths.append(os.path.join(
                        current_app.static_url_path,
                        folder_format+filename))
...
def delete(id):
    ...
    if rental:
        shutil.rmtree('{0}_{1}_{2/}'.format(rental.make,
                      rental.model, rental.category))
    if rental.delete():
